...
#(define mg-log-ports (make-hash-table 8))
...
as an engraver for convenience."
   ...
   ;; MIDGRID PATCH: stock event-listener opens and closes the log file for EVERY
   ;; event. On a big score that is tens of thousands of open/close pairs and
   ;; dominates the run -- BWV 565's listener pass ran past a 300s timeout and
   ;; produced nothing. Keep one port per staff open and force-output instead;
   ;; the content written is byte-identical.
   (let* ((name (filename-from-staffname context))
          (p (or (hash-ref mg-log-ports name #f)
                 (let ((q (open-file name "a")))
                   (hash-set! mg-log-ports name q)
                   q))))
...
%%% MIDGRID PATCH -------------------------------------------------------------
%% Stock event-listener.ly calls (mg-dur-len (ly:event-property ev
%% 'duration)) unguarded. Some events reach the listeners with 'duration = #f --
%% BWV 565's engraving trips this -- and the whole compile then dies with
%% "Wrong type argument (expecting Duration): #f", yielding no log at all.
%% We only consume the "note" and "script" lines (for ornament realisation), so a
...
#(define (format-tempo engraver event)
   ;; MIDGRID PATCH: a TEXTUAL tempo mark -- \tempo "Adagio", of which BWV 565
   ;; has several -- carries no metronome-count, and dividing #f killed the whole
   ;; compile before a single line was logged. There is no numeric tempo to
   ;; report for such a mark, so skip it rather than abort.
   (let ((count (ly:event-property event 'metronome-count)))
     (if (number? count)
         (print-line engraver
                     "tempo"
