...
def build_timemap(total_beats, bpm, rit_beats, rit_amount, agogic, bar_len, prof,
    ...
    # beats outright.
    # The hold goes at the END of the note, in a short window, NOT across the
    # note's whole span. Stretching the span slows down whatever fills it -- and
    # an ornamented fermata note is filled by its ornament, so BWV 565's opening
    # pralls came out as slow, warbling trills instead of a crisp ornament
    # followed by a held note. Bach's sign means: play the ornament at speed,
    # then hold. Stretching a window at the tail does exactly that, and any note
    # sounding at the span's end (the ornament's last note, or a plain note, or
    # the rest of the chord) sustains through it automatically.
    ferm = []
    for fb, fd in sorted(fermatas):
        span = max(fd, 0.05)
        added = max(span * fermata_mult, fermata_min) - span
        if added <= 0: continue
        w = min(span, 0.125)                     # a 32nd: the tail of the note
        ferm.append((fb + span - w, fb + span, (w + added) / w))
    def fermata_scale(b):
        for b0, b1, k in ferm:
            if b0 <= b < b1: return k
        return 1.0
